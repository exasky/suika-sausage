...
export class GameModelClass {
  ...
  constructor(initialSetKey: SetKey = window.INITIAL_SET_KEY || 'sausages') {
    this.currentSetKey = initialSetKey;
    this.itemTypes = getScaledSet(this.currentSetKey);
    this.highScore = readHighScore(this.currentSetKey);
  }
  ...
  getSavedGame(): GameSnapshot & { version: number; setKey: SetKey; } | null {
    try {
...
export function createGameModel(initialSetKey: SetKey = window.INITIAL_SET_KEY || 'sausages') {
