...
export class UiManager {
  ...
  constructor(scene: Phaser.Scene, model: GameModel) {
  ...
  positionUIElements(model: GameModel) {
    if (isMobilePortrait) {
      if (this.uiElements.homeBtnText) this.uiElements.homeBtnText.setText(' 🏠 ').setPosition(5 * SCALE, 6 * SCALE);

      const themeLabel = model.isDarkMode ? ' 🌙 ' : ' ☀️ ';
      if (this.uiElements.themeBtnText) this.uiElements.themeBtnText.setText(themeLabel).setPosition(55 * SCALE, 6 * SCALE);

      const soundLabel = model.isMuted ? ' 🔇 ' : ' 🔊 ';
      if (this.uiElements.soundBtnText) this.uiElements.soundBtnText.setText(soundLabel).setPosition(105 * SCALE, 6 * SCALE);
      ...
      if (this.uiElements.leaderLabel) this.uiElements.leaderLabel.setPosition(10 * SCALE, 45 * SCALE);
      this.uiElements.topScoresTexts.forEach((txt, i) => txt?.setPosition(10 * SCALE, (62 + i * 15) * SCALE));

      if (this.uiElements.nextLabel) this.uiElements.nextLabel.setPosition(BOARD_WIDTH - 110 * SCALE, 45 * SCALE);

      const bottomY = boardY + BOARD_HEIGHT;
      if (this.uiElements.scoreLabel) this.uiElements.scoreLabel.setPosition(15 * SCALE, bottomY + 15 * SCALE);
      if (this.uiElements.scoreText) this.uiElements.scoreText.setPosition(15 * SCALE, bottomY + 30 * SCALE);
      if (this.uiElements.highLabel) this.uiElements.highLabel.setPosition(100 * SCALE, bottomY + 15 * SCALE);
      if (this.uiElements.highScoreText) this.uiElements.highScoreText.setPosition(100 * SCALE, bottomY + 30 * SCALE);
      if (this.uiElements.sausNameText) this.uiElements.sausNameText.setPosition(15 * SCALE, bottomY + 60 * SCALE);

      if (this.uiElements.wheelLabel) {
        this.uiElements.wheelLabel.setPosition(BOARD_WIDTH / 2 + 10 * SCALE, bottomY + 15 * SCALE);
      }
    } else {
      const uiX = BOARD_WIDTH + 15 * SCALE;

      if (this.uiElements.homeBtnText) this.uiElements.homeBtnText.setText(' 🏠 MENU ').setPosition(10 * SCALE, 8 * SCALE);
      if (this.uiElements.timerText) {
      ...
      if (this.uiElements.themeBtnText) this.uiElements.themeBtnText.setText(themeLabel).setPosition(uiX, 12 * SCALE);

      const soundLabel = model.isMuted ? ' 🔇 MUET ' : ' 🔊 SON ';
      if (this.uiElements.soundBtnText) this.uiElements.soundBtnText.setText(soundLabel).setPosition(uiX, 42 * SCALE);

      if (this.uiElements.leaderLabel) this.uiElements.leaderLabel.setPosition(uiX, 75 * SCALE);
      this.uiElements.topScoresTexts.forEach((txt, i) => txt?.setPosition(uiX, (92 + i * 15) * SCALE));

      if (this.uiElements.nextLabel) this.uiElements.nextLabel.setPosition(uiX, 150 * SCALE);

      if (this.uiElements.scoreLabel) this.uiElements.scoreLabel.setPosition(uiX, 275 * SCALE);
      if (this.uiElements.scoreText) this.uiElements.scoreText.setPosition(uiX, 290 * SCALE);
      if (this.uiElements.highLabel) this.uiElements.highLabel.setPosition(uiX, 325 * SCALE);
      if (this.uiElements.highScoreText) this.uiElements.highScoreText.setPosition(uiX, 340 * SCALE);
      if (this.uiElements.sausNameText) this.uiElements.sausNameText.setPosition(uiX, 375 * SCALE);

      if (this.uiElements.wheelLabel) this.uiElements.wheelLabel.setPosition(uiX, 415 * SCALE);
    }
  }
  ...
  drawWheelGraphics(colors) {
    ...
    if (!this.uiElements.wheelGraphics) return;
  ...
  applyTheme(scene: Phaser.Scene, model: GameModel) {
  ...
  updateTimerDisplay(model: GameModel, seconds = model.elapsedTime) {
    if (this.uiElements.timerText) {
